...
def define_variable(self):
    ...
    self.block_schedule_var_by_id_group_surf_rotate_work_dict = {}  # 블록 ID, 그룹, 정반, 회전, 작업별 일정 변수
        ...
        for surface_group_key, work_area in self.work_area_dict.items():
            ...
            for rotate in rotation_list:
                ...
                self.block_time_var_by_id_group_surf_rotate_dict[var_key] = self.cpmodel.interval_var(
                    size=block.processing_time,  # 블록 적치 시간
                    optional=True,
                ...
                # 각 작업별 일정 변수 생성
                for work in work_list:
                    var_key_work = (block_id, surface_group_key, surface_id, rotate, work)

                    if work == 'in':
                        # 반입 일정
                        self.block_schedule_var_by_id_group_surf_rotate_work_dict[
                            var_key_work] = self.cpmodel.interval_var(
                            start=(0, 10),
                            size=1,
                            optional=True,
                    ...
                    self.block_schedule_var_list_by_id_work_dict[(block_id, work)].append(
                        self.block_schedule_var_by_id_group_surf_rotate_work_dict[var_key_work]
                    )

                # 위치 변수와 일정 변수 간의 연결 (presence_of)
                # 위치 변수들 간의 연결
                self.cpmodel.add(
                ...
                self.cpmodel.add(
                    self.cpmodel.presence_of(self.block_x_var_by_id_group_surf_rotate_dict[var_key]) ==
                    self.cpmodel.presence_of(self.block_schedule_var_by_id_group_surf_rotate_work_dict[
                                                 (block_id, surface_group_key, surface_id, rotate, 'store')])
                )

                # 시간축 변수와 store 작업 변수 간의 시간 동기화
                self.cpmodel.add(
                    self.cpmodel.start_of(self.block_time_var_by_id_group_surf_rotate_dict[var_key]) ==
                    self.cpmodel.start_of(self.block_schedule_var_by_id_group_surf_rotate_work_dict[
                                              (block_id, surface_group_key, surface_id, rotate, 'store')])
                )
                self.cpmodel.add(
                    self.cpmodel.end_of(self.block_time_var_by_id_group_surf_rotate_dict[var_key]) ==
                    self.cpmodel.end_of(self.block_schedule_var_by_id_group_surf_rotate_work_dict[
                                            (block_id, surface_group_key, surface_id, rotate, 'store')])
                )
                ...
                for i in range(len(work_list) - 1):
                    self.cpmodel.add(
                        self.cpmodel.presence_of(self.block_schedule_var_by_id_group_surf_rotate_work_dict[
                                                     (
                                                     block_id, surface_group_key, surface_id, rotate, work_list[i])]) ==
                        self.cpmodel.presence_of(self.block_schedule_var_by_id_group_surf_rotate_work_dict[
                                                     (block_id, surface_group_key, surface_id, rotate,
                                                      work_list[i + 1])])
                    )
            ...
            for rotate in rotation_list:
                ...
                self.cpmodel.add(
                    self.cpmodel.end_at_start(
                        self.block_schedule_var_by_id_group_surf_rotate_work_dict[
                            (block_id, surface_group_key, surface_id, rotate, 'in')],
                        self.block_schedule_var_by_id_group_surf_rotate_work_dict[
                            (block_id, surface_group_key, surface_id, rotate, 'store')]
                    )
                )
                ...
                self.cpmodel.add(
                    self.cpmodel.end_at_start(
                        self.block_schedule_var_by_id_group_surf_rotate_work_dict[
                            (block_id, surface_group_key, surface_id, rotate, 'store')],
                        self.block_schedule_var_by_id_group_surf_rotate_work_dict[
                            (block_id, surface_group_key, surface_id, rotate, 'out')]
                    )
                )
                ...
                self.cpmodel.add(
                    self.cpmodel.end_at_start(
                        self.block_schedule_var_by_id_group_surf_rotate_work_dict[
                            (block_id, surface_group_key, surface_id, rotate, 'out')],
                        self.block_schedule_var_by_id_group_surf_rotate_work_dict[
                            (block_id, surface_group_key, surface_id, rotate, 'TO')]
                    )
                )
                ...
                self.cpmodel.add(
                    self.cpmodel.end_at_start(
                        self.block_schedule_var_by_id_group_surf_rotate_work_dict[
                            (block_id, surface_group_key, surface_id, rotate, 'TO')],
                        self.block_schedule_var_by_id_group_surf_rotate_work_dict[
                            (block_id, surface_group_key, surface_id, rotate, 'PE')]
                    )
                )
