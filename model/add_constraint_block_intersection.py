def add_constraint_block_intersection(self):
    ...
    for i, block_key1 in enumerate(self.block_keys):
        ...
        for surface_group_key, work_area in self.work_area_dict.items():
            ...
            for key, value in self.unavailable_work_area_x_var_by_id_dict.items():
                if surface_group_key == key[0]:
                    for rotate1 in self.rotation_list:
                        # 첫 번째 블록 변수 키
                        var_key1 = (block_id1, surface_group_key, surface_id, rotate1)

                        if var_key1 not in self.block_x_var_by_id_group_surf_rotate_dict:
                            continue
                        ...
                        x_var2 = self.unavailable_work_area_x_var_by_id_dict[key]
                        y_var2 = self.unavailable_work_area_y_var_by_id_dict[key]
                        time_var2 = self.unavailable_work_area_time_var_by_id_dict[key]
